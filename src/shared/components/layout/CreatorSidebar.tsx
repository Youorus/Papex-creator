...
export const creatorNavigation = [
  ...
  { name: "Mes Documents", href: "/my-space/contracts", icon: FileText },
    ...
    >
      <div className="flex items-center justify-between h-20 px-4 border-b border-border/50">
        {!isCollapsed && (
          ...
          >
            ...
            <div className="flex flex-col">
              <span className="font-bold text-lg leading-tight tracking-tight text-primary">Ambassadeur</span>
              <span className="text-[10px] font-semibold tracking-widest text-muted-foreground uppercase">Papex Creator</span>
            </div>
          </motion.div>
        )}
        {isCollapsed && (
        ...
        <Button
          ...
          className={cn("absolute -right-4 top-6 h-8 w-8 rounded-full border bg-background shadow-md hover:bg-accent text-foreground")}
        >
      ...
      <div className="p-4 border-t border-border/50">
        {!isCollapsed && creatorProfile && (
          <div className="mb-4 px-3 py-3 bg-accent/30 rounded-xl border border-border/50">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Session active</p>
            <p className="text-sm font-black text-foreground truncate">{creatorProfile.full_name}</p>
          </div>
        )}
        <motion.button
          whileHover={{ scale: 1.02 }}
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="left" className="w-[280px] p-0 flex flex-col bg-background border-border">
        <SheetTitle className="sr-only">Menu Ambassadeur</SheetTitle>
        <div className="flex items-center space-x-3 w-full h-20 px-6 border-b border-border/50">
          ...
          />
          <div className="flex flex-col">
            <span className="font-bold text-base leading-tight tracking-tight text-primary">Ambassadeur</span>
            <span className="text-[9px] font-semibold tracking-widest text-muted-foreground uppercase">Papex Creator</span>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-border/50">
          {creatorProfile && (
            <div className="mb-4 px-4 py-3 bg-accent/30 rounded-xl">
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter mb-1">Ambassadeur</p>
              <p className="text-sm font-bold text-foreground">{creatorProfile.full_name}</p>
            </div>
          )}
          <Button
            variant="ghost"
